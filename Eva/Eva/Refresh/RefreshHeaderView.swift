...
class RefreshHeaderView: RefreshBaseView {
    class func footer()->RefreshHeaderView{
        var footer:RefreshHeaderView  = RefreshHeaderView(frame: CGRectMake(0, 0,   UIScreen.mainScreen().bounds.width, RefreshConst().RefreshViewHeight))
        return footer
    }
    ...
    var lastUpdateTime:NSDate = NSDate(){
        ...
        didSet{
            NSUserDefaults.standardUserDefaults().setObject(lastUpdateTime, forKey: RefreshConst().RefreshHeaderTimeKey)
    ...
    override init(frame: CGRect) {
        ...
        lastUpdateTimeLabel.textColor = RefreshConst().RefreshLabelTextColor
        ...
        if  NSUserDefaults.standardUserDefaults().objectForKey(RefreshConst().RefreshHeaderTimeKey) == nil {
            self.lastUpdateTime = NSDate()
        } else {
            self.lastUpdateTime = NSUserDefaults.standardUserDefaults().objectForKey(RefreshConst().RefreshHeaderTimeKey) as! NSDate
        }
        self.updateTimeLabel()
    }
...
//        fatalError("init(coder:) has not been implemented")
    ...
    override func observeValueForKeyPath(keyPath: String, ofObject object: AnyObject, change: [NSObject : AnyObject], context: UnsafeMutablePointer<Void>) {
        ...
        if RefreshConst().RefreshContentOffset == keyPath {
    ...
    didSet{
        switch State{
        case .Normal:
            self.statusLabel.text = RefreshConst().RefreshHeaderPullToRefresh
            if RefreshState.Refreshing == oldState {
                self.arrowImage.transform = CGAffineTransformIdentity
                self.lastUpdateTime = NSDate()
                UIView.animateWithDuration(RefreshConst().RefreshSlowAnimationDuration, animations: {
            ...
            break
        case .Pulling:
            self.statusLabel.text = RefreshConst().RefreshHeaderReleaseToRefresh
            UIView.animateWithDuration(RefreshConst().RefreshSlowAnimationDuration, animations: {
                 self.arrowImage.transform = CGAffineTransformMakeRotation(CGFloat(M_PI ))
                })
            break
        case .Refreshing:
            self.statusLabel.text =  RefreshConst().RefreshHeaderRefreshing
            UIView.animateWithDuration(RefreshConst().RefreshSlowAnimationDuration, animations: {

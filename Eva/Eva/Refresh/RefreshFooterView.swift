...
class RefreshFooterView: RefreshBaseView {
    class func footer()->RefreshFooterView{
        var footer:RefreshFooterView  = RefreshFooterView(frame: CGRectMake(CGFloat(0), CGFloat(0), UIScreen.mainScreen().bounds.width, RefreshConst().RefreshViewHeight))
        return footer
    }
    ...
    override func observeValueForKeyPath(keyPath: String, ofObject object: AnyObject, change: [NSObject : AnyObject], context: UnsafeMutablePointer<Void>) {
        ...
        if RefreshConst().RefreshContentSize == keyPath{
            adjustFrameWithContentSize()
        }else if RefreshConst().RefreshContentOffset == keyPath {
    ...
    didSet{
        switch State{
        case .Normal:
            self.statusLabel.text = RefreshConst().RefreshFooterPullToRefresh
            ...
            break
        case .Pulling:
            self.statusLabel.text = RefreshConst().RefreshFooterReleaseToRefresh
            UIView.animateWithDuration(RefreshConst().RefreshSlowAnimationDuration, animations: {
